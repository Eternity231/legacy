...
void math::clamp_skeet(vec3_t& ang) {
...
vec3_t math::vec_angle(vec3_t vec) {
    vec3_t ret;
    ...
    if (vec.x == 0.0f && vec.y == 0.0f) {
        ...
        ret.x = rad_to_deg(std::atan2f(-vec.z, vec.length_2d()));
        ret.y = rad_to_deg(std::atan2f(vec.y, vec.x));
    ...
    math::clamp_skeet(ret);

    return ret;
}

vec3_t math::calc_angle(const vec3_t& from, const vec3_t& to) {
